...
class InbounceSnsTestCase(TestCase):
    def test_happy_path(self):
        ...
        handler = MyReceiver(mail_obj=mail_obj, receipt=receipt, raw_message=notification)
    ...
    def test_bad_content(self):
        mail_obj, content, receipt, notification = get_mock_received_sns()
        handler = SnsHandler(mail_obj=mail_obj, receipt=receipt, raw_message=notification)
        ...
        with self.assertLogs("django_ses", level="INFO") as cm:
            ...
            self.assertEqual(
                cm.output[-1], "ERROR:django_ses.inbound:Content couldn't be decoded with base64. Invalid content?"
            )
...
class InbounceS3TestCase(TestCase):
    ...
    def test_wrong_handler(self):
        mail_obj, content, receipt, notification = get_mock_received_sns()
        handler = S3Handler(mail_obj=mail_obj, receipt=receipt, raw_message=notification)
        ...
        with self.assertLogs("django_ses", level="INFO") as cm:
            ...
            self.assertEqual(
                cm.output[-1], "ERROR:django_ses.inbound:Received action type (SNS) can't be handled by this handler"
            )
