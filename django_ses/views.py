...
def dashboard(request):
    ...
    """
    warnings.warn(
        "This view will be removed in future versions. Consider using DashboardView instead", DeprecationWarning
    )
...
def handle_bounce(request):
    ...
    if notification.get("Type") in ("SubscriptionConfirmation", "UnsubscribeConfirmation"):
        ...
        try:
            ...
            logger.warning(
                'Received bounce with bad JSON: "%s"',
                e,
                extra={
                    "notification": notification,
                },
...
class SESEventWebhookView(View):
    ...
    def handle_bounce(self, notification, message):
        self._handle_event(
            event_name="bounce", signal=signals.bounce_received, notification=notification, message=message
        )

    def handle_complaint(self, notification, message):
        self._handle_event(
            event_name="complaint", signal=signals.complaint_received, notification=notification, message=message
        )

    def handle_delivery(self, notification, message):
        self._handle_event(
            event_name="delivery", signal=signals.delivery_received, notification=notification, message=message
        )

    def handle_send(self, notification, message):
        self._handle_event(event_name="send", signal=signals.send_received, notification=notification, message=message)

    def handle_open(self, notification, message):
        self._handle_event(event_name="open", signal=signals.open_received, notification=notification, message=message)

    def handle_click(self, notification, message):
        self._handle_event(
            event_name="click", signal=signals.click_received, notification=notification, message=message
        )
