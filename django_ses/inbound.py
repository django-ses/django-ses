...
class S3Handler(BaseHandler):
    def check_action_compatibility(self):
        ...
        if action_type != "S3":
            logger.error(f"Received action type ({action_type}) can't be handled by this handler")
...
class SnsHandler(BaseHandler):
    def check_action_compatibility(self):
        ...
        if action_type != "SNS" or encoding != "BASE64":
            logger.error(
                f"Received action type ({action_type}) or encoding ({encoding}) can't be handled by this handler"
            )
            raise UnprocessableError
    ...
    def prepare_content(self, content):
        try:
            ...
            logger.error("Content couldn't be decoded with base64. Invalid content?")
            raise UnprocessableError
