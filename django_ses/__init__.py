...
# These would be nice to make class-level variables, but the backend is re-created
# for each outgoing email/batch. recent_send_times also is not going to work quite
# right if there are multiple email backends with different rate limits returned by
# SES, but that seems like it would be rare.
cached_rate_limits = {}
recent_send_times = []
...
class SESBackend(BaseEmailBackend):
    ...
    def send_messages(self, email_messages):
        ...
        for message in email_messages:
            # Automatic throttling. Assumes that this is the only SES client currently operating.
            # The AWS_SES_AUTO_THROTTLE setting is a factor to apply to the rate limit,
            # with a default of 0.5 to stay well below the actual SES throttle. Set the setting
            # to 0 or None to disable throttling.
            if self._throttle:
                global recent_send_times
                
                now = datetime.now()
                
                # Get and cache the current SES max-per-second rate limit returned by the SES API.
                rate_limit = self.get_rate_limit()
                    
                # Prune from recent_send_times anything more than a few seconds ago. Even
                # though SES reports a maximum per-second, the way they enforce the limit
                # may not be on a one-second window. To be safe, we use a two-second
                # window (but allow 2 times the rate limit) and then also have a default rate limit
                # factor of 0.5 so that we really limit the one-second amount in two seconds.
                window = 2.0 # seconds
                ...
                # If the number of recent send times in the last 1/_throttle seconds exceeds the rate limit,
                # then add a delay. Since I'm not sure how Amazon determines at exactly what
                # point to throttle, better be safe than sorry and let in, say, half of the allowed rate.
    ...
    def get_rate_limit(self):
        ...
        try:
            ...
            max_per_second = quota_dict['GetSendQuotaResponse']['GetSendQuotaResult']['MaxSendRate']
